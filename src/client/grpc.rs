...
use crate::proto::hoosatd_message::Payload;
use crate::proto::rpc_client::RpcClient;
use crate::proto::{
    GetBlockTemplateRequestMessage, GetInfoRequestMessage, HoosatdMessage, NotifyBlockAddedRequestMessage,
    ...
    async fn listen(&mut self, miner: &mut MinerManager) -> Result<(), Error> {
        while let Some(msg) = self.stream.message().await? {
            match msg.payload {
                ...
                None => warn!("htnd message payload is empty"),
            }
        }
        Ok(())
    }
...
impl hoohashHandler {
    ...
    fn create_block_channel(send_channel: Sender<HoosatdMessage>) -> (Sender<BlockSeed>, BlockHandle) {
        // HoosatdMessage::submit_block(block)
            ...
            tokio::spawn(async move {
                ReceiverStream::new(recv)
                    .map(|block_seed| match block_seed {
                        FullBlock(block) => HoosatdMessage::submit_block(*block),
    ...
    async fn client_send(&self, msg: impl Into<HoosatdMessage>) -> Result<(), SendError<HoosatdMessage>> {
        self.send_channel.send(msg.into()).await
    }

    async fn client_get_block_template(&mut self) -> Result<(), SendError<HoosatdMessage>> {
    ...
    async fn handle_message(&mut self, msg: Payload, miner: &mut MinerManager) -> Result<(), Error> {
        match msg {
            ...
            Payload::GetInfoResponse(info) => {
                info!("htnd version: {}", info.server_version);
                let htnd_version = Version::parse(&info.server_version)?;
                let update_version = Version::parse(VERSION_UPDATE)?;
                match htnd_version >= update_version {
            ...
            Payload::NotifyBlockAddedResponse(res) => match res.error {
                None => info!("Registered for block notifications (upgrade your htnd for better experience)"),
