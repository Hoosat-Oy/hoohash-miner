...
#[derive(Parser, Debug)]
#[clap(name = "hoohash-miner", version, about = "A Hoohash high performance miner", term_width = 0)]
pub struct Opt {
    ...
    #[clap(short, long, help = "HTND port [default: Mainnet = 42110, Testnet = 16211]")]
    port: Option<u16>,

    #[clap(long, help = "Use testnet instead of mainnet [default: false]")]
    testnet: bool,
    #[clap(short = 't', long = "threads", help = "Amount of miner threads to launch [default: 0]")]
    pub num_threads: Option<u16>,
    #[clap(
        long = "mine-when-not-synced",
        help = "Mine even when HTND says it is not synced",
        long_help = "Mine even when HTND says it is not synced, only useful when passing `--allow-submit-block-when-not-synced` to HTND  [default: false]"
    )]
...
impl Opt {
    pub fn process(&mut self) -> Result<(), Error> {
        //self.gpus = None;
        if self.htnd_address.is_empty() {
            self.htnd_address = "127.0.0.1".to_string();
        }

        if !self.htnd_address.contains("://") {
            let port_str = self.port().to_string();
            let (htnd, port) = match self.htnd_address.contains(':') {
                true => self.htnd_address.split_once(':').expect("We checked for `:`"),
                false => (self.htnd_address.as_str(), port_str.as_str()),
            };
            self.htnd_address = format!("grpc://{}:{}", htnd, port);
        }
        log::info!("HTND address: {}", self.htnd_address);
        ...
        self.devfund_address = String::from("hoosat:qrt7lyuplddukhjnm8wlu9dy3npz2j58a6qhpm9cmdawfw5ymmhxgpvmwfnsp");
